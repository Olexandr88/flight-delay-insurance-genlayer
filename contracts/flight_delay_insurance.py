...
class FlightDelayInsurance(gl.Contract):
    ...
    def buy_policy(
        ...
        # The derived URL binds BOTH the flight number and the date, not
        # just the number — otherwise the same URL could be reused across
        # different dates for the same flight, letting a policyholder
        # point validators at evidence for the wrong day.
        status_url = f"https://www.flightaware.com/live/flight/{flight_number}/history/{flight_date}"
...
First, confirm the evidence below is actually reporting on flight
{flight_number} for the date {flight_date} specifically — not a
different date's flight history or a different flight. If the evidence
does not clearly confirm both the flight number and the date, respond
with verdict "undetermined" rather than guessing from a mismatched page.

If the flight and date are confirmed, read the flight-status evidence
and determine the flight's actual arrival delay in minutes relative to
its scheduled arrival time.

Respond using ONLY the following JSON format, nothing else:
{{
  "verdict": str,        // "delayed" if delay_minutes >= {DELAY_THRESHOLD_MINUTES}, "on_time" if it flew with a smaller delay or none, "undetermined" if the evidence does not clearly confirm this flight and date, or does not clearly show the flight's status
  "delay_minutes": int,  // best estimate of the delay in minutes (0 if on time or undetermined)
  "reasoning": str       // one or two sentences citing what in the evidence supports this, including confirmation of the flight number and date
}}
...
"""
        ...
        def leader_fn():
            ...
            # Deterministic authentication step, run identically by every
            # validator, BEFORE any LLM judgment is even attempted: the
            # fetched page must actually contain confirmation of both the
            # flight number and the flight date this policy covers. This
            # closes the gap between "the URL is derived correctly from
            # stored fields" and "the page content genuinely refers to
            # this flight on this date" — a page that failed to load the
            # right content, redirected silently, or otherwise doesn't
            # confirm both fields is rejected here, never reaching the
            # LLM at all.
            if flight_number not in web_data or flight_date not in web_data:
                return {
                    "verdict": "undetermined",
                    "delay_minutes": 0,
                    "reasoning": (
                        f"Evidence page did not contain confirmation of "
                        f"both flight {flight_number} and date {flight_date}; "
                        f"refusing to adjudicate against unconfirmed evidence."
                    ),
                }
